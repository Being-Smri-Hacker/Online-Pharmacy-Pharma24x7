	...
	if(isset($_POST['add'])){
		...
		if($row['numrows'] > 0){
			...
			try{
				...
				$stmt->execute(['email'=>$email, 'password'=>$password, 'firstname'=>$firstname, 'lastname'=>$lastname, 'address'=>$address, 'contact'=>$contact, 'photo'=>$filename, 'status'=>$status, 'created_on'=>$now,'type'=>1]);
