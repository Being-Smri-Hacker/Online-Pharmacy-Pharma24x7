							...
							if (validateExpiryDate($expiryDate)){
									...
									if (validateCVV($cvv)){
										...
										if($row['numrows']==0){
											...
											$stmt=$conn->prepare("INSERT INTO subs_user(email,credit_card_no,expiry_date,cvv,subs_amt,user_id,subs_date) values (:email,:ccno,:ed,:cvv,:subsamt,:userid,:sdate)");
											$stmt->execute(['email'=>$email,'ccno'=>$encryptedccno,'ed'=>$expiryDate,'cvv'=>$encryptedcvv,'subsamt'=>$subsamt,'userid'=>$userid,'sdate'=>$sdate]);
										}
								}
								else {
